...
const DROPS_PER_BMC = new Decimal('1e6');
const MAX_NETWORK_DROPS = new Decimal('1e17');
const MIN_BMC = new Decimal('1e-6')
const MAX_BMC = MAX_NETWORK_DROPS.dividedBy(DROPS_PER_BMC);
...
const parsers = {
  string(str) {
    ...
    return [new Decimal(str).dividedBy(DROPS_PER_BMC), Currency.BMC];
  },
  object(object) {
...
const Amount = makeClass({
  Amount(value, currency, issuer, validate = true) {
    ...
    this.currency = currency || Currency.BMC;
  ...
  statics: {
    ...
    fromParser(parser) {
      ...
      const bmcValue = drops.dividedBy(DROPS_PER_BMC);
      return new this(bmcValue, Currency.BMC, null, false);
    }
  },
  assertValueIsValid() {
    ...
    if (!this.isZero()) {
      if (this.isNative()) {
        ...
        if (abs.lt(MIN_BMC) || abs.gt(MAX_BMC)) {
          // value is in BMC scale, but show the value in canonical json form
          raiseIllegalAmountError(this.value.times(DROPS_PER_BMC))
        }
  ...
  valueString() {
    return (this.isNative() ? this.value.times(DROPS_PER_BMC) : this.value)
      .toString();
  },
  toBytesSink(sink) {
