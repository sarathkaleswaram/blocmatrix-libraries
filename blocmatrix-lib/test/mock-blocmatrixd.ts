...
import fixtures from './fixtures/blocmatrixd'
import addresses from './fixtures/addresses.json'
import hashes from './fixtures/hashes.json'
import transactionsResponse from './fixtures/blocmatrixd/account-tx'
import accountLinesResponse from './fixtures/blocmatrixd/account-lines'
import accountObjectsResponse from './fixtures/blocmatrixd/account-objects'
import fullLedger from './fixtures/blocmatrixd/ledger-full-38129.json'
...
export function createMockBlocmatrixd(port) {
    ...
    if (request.account === addresses.ACCOUNT) {
      ...
      const response = Object.assign(
        {},
        {
          ...
          request:
            // This will be inaccurate, but that's OK because this is just a mock blocmatrixd
            {
  ...
  mock.on('request_book_offers', function(request, conn) {
    if (request.taker_pays.issuer === 'rp8rJYTpodf8qbSCHVTNacf8nSW8mRakFw') {
      conn.send(createResponse(request, fixtures.book_offers.bmc_usd))
    } else if (
      request.taker_gets.issuer === 'rp8rJYTpodf8qbSCHVTNacf8nSW8mRakFw'
    ) {
      conn.send(createResponse(request, fixtures.book_offers.usd_bmc))
    } else if (
      ...
      const blocmatrixdDir = 'test/fixtures/blocmatrixd'
      if (!requestsCache) {
        requestsCache = fs.readdirSync(blocmatrixdDir + '/requests')
      }
      for (var i = 0; i < requestsCache.length; i++) {
        const file = requestsCache[i]
        const json = fs.readFileSync(blocmatrixdDir + '/requests/' + file, 'utf8')
        ...
        if (JSON.stringify(requestWithoutId) === JSON.stringify(r)) {
          const responseFile =
          blocmatrixdDir + '/responses/' + file.split('.')[0] + '-res.json'
  ...
  mock.on('request_blocmatrix_path_find', function(request, conn) {
    let response = null
    ...
    if (request.source_account === 'rB2NTuTTS3eNCsWxZYzJ4wqRqxNLZqA9Vx') {
      ...
      response = createResponse(request, {
        ...
        result: {
          ...
          destination_currencies: ['EUR', 'BMC'],
          full_reply: true,
          id: 2,
      ...
      response = createResponse(request, fixtures.path_find.BmctoBmc, {
