...
import {isValidXAddress, isValidClassicAddress} from 'blocmatrix-address-codec'
...
class BlocmatrixAPI extends EventEmitter {
  _feeCushion: number
  _maxFeeBMC: string

  // New in > 0.21.0
  // non-validated ledger versions are allowed, and passed to Blocmatrixd as-is.
  ...
  constructor(options: APIOptions = {}) {
    ...
    this._maxFeeBMC = options.maxFeeBMC || '2'
   ...
   *
   * See https://blocmatrix.com/build/Blocmatrixd-apis/#markers-and-pagination
   */
   ...
   * general use. Instead, use blocmatrixd's built-in pagination and make multiple
   * requests as needed.
   */
  async _requestAll(
    command: 'account_offers',
  ...
  // BlocmatrixAPI.deriveClassicAddress (static) is a new name for api.deriveAddress
  ...
  bmcToDrops = bmcToDrops
  dropsToBmc = dropsToBmc
  BlocmatrixTimeToISO8601 = blocmatrixTimeToISO8601
  iso8601ToBlocmatrixTime = iso8601ToBlocmatrixTime
  txFlags = txFlags
