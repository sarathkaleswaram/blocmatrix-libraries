...
function getBMCBalance(
  ...
  return connection
    .request(request)
    .then(data => common.dropsToBmc(data.account_data.Balance))
}
...
/**
 *  Order two Blocmatrixd transactions based on their ledger_index.
 *  If two transactions took place in the same ledger, sort
 *  them based on TransactionIndex
 *  See: https://developers.Blocmatrix.com/transaction-metadata.html
 */
...
function ensureLedgerVersion(this: BlocmatrixAPI, options: any): Promise<object> {
  if (
    Boolean(options) &&
