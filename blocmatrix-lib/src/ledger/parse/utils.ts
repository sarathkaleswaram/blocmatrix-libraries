...
import transactionParser from 'blocmatrix-lib-transactionparser'
...
function adjustQualityForBMC(
  quality: string,
  takerGetsCurrency: string,
  takerPaysCurrency: string
) {
  // quality = takerPays.value/takerGets.value
  // using drops (1e-6 BMC) for BMC values
  const numeratorShift = takerPaysCurrency === 'BMC' ? -6 : 0
  const denominatorShift = takerGetsCurrency === 'BMC' ? -6 : 0
...
function parseTimestamp(blocmatrixTime?: number | null): string | undefined {
  if (typeof blocmatrixTime !== 'number') {
    return undefined
  }
  return common.blocmatrixTimeToISO8601(blocmatrixTime)
}
