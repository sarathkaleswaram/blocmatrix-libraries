...
// blocmatrixd 'account_lines' returns a different format for
// trustlines than 'tx'
function parseAccountTrustline(trustline: Trustline): FormattedTrustline {
  const specification = removeUndefined({
    ...
    ripplingDisabled: trustline.no_blocmatrix || undefined,
    frozen: trustline.freeze || undefined,
    authorized: trustline.authorized || undefined
  })
  // blocmatrixd doesn't provide the counterparty's qualities
  const counterparty = removeUndefined({
    limit: trustline.limit_peer,
    ripplingDisabled: trustline.no_blocmatrix_peer || undefined,
