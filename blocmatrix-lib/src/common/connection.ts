...
class Connection extends EventEmitter {
  ...
  _onOpen() {
    ...
    return this.request(request).then((data: any) => {
      if (_.isEmpty(data) || !data.ledger_index) {
        // Blocmatrixd instance doesn't have validated ledgers
        return this._disconnect(false).then(() => {
          throw new BlocmatrixdNotInitializedError('Blocmatrixd not initialized')
        })
      }
          ...
          reject(
            new ConnectionError(
              `Error: connect() timed out after ${this._config.connectionTimeout} ms. ` +
                `If your internet connection is working, the Blocmatrixd server may be blocked or inaccessible.`
            )
          )
  ...
  reconnect() {
    ...
    // See: https://github.com/Blocmatrix/Blocmatrix-lib/pull/1101#issuecomment-565360423
    this.emit('reconnect')
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(eventName, response => {
        if (response.status === 'error') {
          _reject(
            new BlocmatrixdError(response.error_message || response.error, response)
          )
