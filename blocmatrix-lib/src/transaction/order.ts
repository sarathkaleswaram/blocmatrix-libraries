...
function createOrderTransaction(
  ...
  const takerPays = utils.common.toBlocmatrixdAmount(
    order.direction === 'buy' ? order.quantity : order.totalPrice
  )
  const takerGets = utils.common.toBlocmatrixdAmount(
  ...
  if (order.expirationTime !== undefined) {
    txJSON.Expiration = iso8601ToBlocmatrixTime(order.expirationTime)
  }
