...
function createEscrowCreationTransaction(
  ...
  const txJSON: any = {
    ...
    Amount: bmcToDrops(payment.amount)
  }
  ...
  if (payment.allowCancelAfter !== undefined) {
    txJSON.CancelAfter = iso8601ToBlocmatrixTime(payment.allowCancelAfter)
  }
  if (payment.allowExecuteAfter !== undefined) {
    txJSON.FinishAfter = iso8601ToBlocmatrixTime(payment.allowExecuteAfter)
  }
