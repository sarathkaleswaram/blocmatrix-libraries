...
import binaryCodec from 'blocmatrix-binary-codec'
...
/**
 *  Check that a given transaction fee does not exceed maxFeeBMC (in drops).
 *
 *  See https://bmcl.org/Blocmatrixapi-reference.html#parameters
 *
 *  @param {BlocmatrixAPI} api A BlocmatrixAPI instance.
 *  @param {string} txFee The transaction fee in drops, encoded as a string.
 *
 *  @returns {void} This method does not return a value, but throws an error if the check fails.
 */
function checkFee(api: BlocmatrixAPI, txFee: string): void {
  const fee = new BigNumber(txFee)
  const maxFeeDrops = bmcToDrops(api._maxFeeBMC)
  if (fee.isGreaterThan(maxFeeDrops)) {
    throw new utils.common.errors.ValidationError(
      `"Fee" should not exceed "${maxFeeDrops}". ` +
        'To use a higher fee, set `maxFeeBMC` in the BlocmatrixAPI constructor.'
    )
  }
}
