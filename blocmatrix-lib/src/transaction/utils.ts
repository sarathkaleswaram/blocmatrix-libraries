...
import {xAddressToClassicAddress, isValidXAddress} from 'blocmatrix-address-codec'
...
function formatPrepareResponse(txJSON: any): Prepare {
  const instructions = {
    fee: common.dropsToBmc(txJSON.Fee),
 ...
 *
 *  See https://bmcl.org/transaction-malleability.html
 *
...
function prepareTransaction(
  ...
  function prepareFee(): Promise<void> {
    ...
    // Furthermore, txJSON.Fee is in drops while instructions.fee is in BMC, which would just add to
    ...
    if (instructions.fee !== undefined) {
      ...
      if (fee.isGreaterThan(api._maxFeeBMC)) {
        return Promise.reject(
          new ValidationError(
            `Fee of ${fee.toString(10)} BMC exceeds ` +
              `max of ${api._maxFeeBMC} BMC. To use this fee, increase ` +
              '`maxFeeBMC` in the BlocmatrixAPI constructor.'
          )
        )
      }
      newTxJSON.Fee = scaleValue(
        common.bmcToDrops(instructions.fee),
        multiplier
      )
      return Promise.resolve()
    }
    ...
    return api.getFee(cushion).then(fee => {
      return api.connection.getFeeRef().then(feeRef => {
        ...
        const feeDrops = common.bmcToDrops(fee)
        const maxFeeBMC = instructions.maxFee
          ? BigNumber.min(api._maxFeeBMC, instructions.maxFee)
          : api._maxFeeBMC
        const maxFeeDrops = common.bmcToDrops(maxFeeBMC)
