...
var dropsToBMC = require('./utils').dropsToBMC
...
function parseBMCQuantity(node, valueParser) {
  ...
  return {
    ...
    balance: {
      counterparty: '',
      currency: 'BMC',
      value: dropsToBMC(value).toString()
    }
  }
}
...
function parseQuantities(metadata, valueParser) {
  var values = normalizeNodes(metadata).map(function(node) {
    if (node.entryType === 'AccountRoot') {
      return [parseBMCQuantity(node, valueParser)]
    } else if (node.entryType === 'BlocmatrixState') {
