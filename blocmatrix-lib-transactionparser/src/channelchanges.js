...
function summarizePaymentChannel(node) {
  ...
  const summary = {
    ...
    // BMC from the channel. This comes from the Destination field of the transaction
    // that created the channel.
    destination: final.Destination,

    // Total BMC, in drops, that has been allocated to this channel.
    // This includes BMC that has been paid to the destination address.
    ...
    // Total BMC, in drops, already paid out by the channel.
    // The difference between this value and the Amount field is how much BMC can still
  ...
  if (prev.Amount) {
    // The change in the number of BMC drops allocated to this channel.
  ...
  if (prev.Balance) {
    // The change in the number of BMC drops already paid out by the channel.
