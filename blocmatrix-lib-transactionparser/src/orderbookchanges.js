...
var lsfSell = 0x00020000 // see "lsfSell" flag in blocmatrixd source code
...
function blocmatrixToUnixTimestamp(rpepoch) {
...
function getExpirationTime(node) {
  ...
  return (new Date(blocmatrixToUnixTimestamp(expirationTime))).toISOString()
}
...
function getQuality(node) {
  ...
  var takerGetsCurrency = takerGets.currency || 'BMC'
  var takerPaysCurrency = takerPays.currency || 'BMC'
 ...
 * Returns changes grouped by Blocmatrix account.
 *
 *  @param {Object} metadata - Transaction metadata as return by blocmatrix-lib
 *  @returns {Object} - Orderbook changes grouped by Blocmatrix account
 *
 */
