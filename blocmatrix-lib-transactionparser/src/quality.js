...
the quotient of TakerPays/TakerGets. It uses drops (1e-6 BMC) for BMC values.
*/

function adjustQualityForBMC(quality, takerGetsCurrency, takerPaysCurrency) {
  var numeratorShift = (takerPaysCurrency === 'BMC' ? -6 : 0)
  var denominatorShift = (takerGetsCurrency === 'BMC' ? -6 : 0)
...
function parseQuality(qualityHex, takerGetsCurrency, takerPaysCurrency) {
  ...
  return adjustQualityForBMC(quality, takerGetsCurrency, takerPaysCurrency)
}
