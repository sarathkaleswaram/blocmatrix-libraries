...
const codecOptions = {
  ...
  alphabet: 'bpshnaf39wBUDNEGHJKLM4PQRST7VWXYZ2rcdeCg65jkm8oFqi1tuvAxyz'
}

const codecWithBmcAlphabet = new Codec(codecOptions)

export const codec = codecWithBmcAlphabet
...
export function encodeSeed(entropy: Buffer, type: 'ed25519' | 'secp256k1'): string {
  ...
  return codecWithBmcAlphabet.encode(entropy, opts)
}
...
export function decodeSeed(seed: string, opts: {
  ...
  return codecWithBmcAlphabet.decode(seed, opts)
}

export function encodeAccountID(bytes: Buffer): string {
  const opts = {versions: [ACCOUNT_ID], expectedLength: 20}
  return codecWithBmcAlphabet.encode(bytes, opts)
}
...
export function decodeAccountID(accountId: string): Buffer {
  const opts = {versions: [ACCOUNT_ID], expectedLength: 20}
  return codecWithBmcAlphabet.decode(accountId, opts).bytes
}
...
export function decodeNodePublic(base58string: string): Buffer {
  ...
  return codecWithBmcAlphabet.decode(base58string, opts).bytes
}
